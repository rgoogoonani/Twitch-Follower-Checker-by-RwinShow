...
try:
    ...
    from twitchAPI.oauth import UserAuthenticator
    from twitchAPI.types import AuthScope
from threading import Thread, active_count, activeCount
import json
import time
try:
	import  requests
...
def check_follows(id_list_input, streamer_id, streamer_name):
    ...
    for user in id_list_input:
        i+=1
        try:
            Thread(target=check_follows2,args=[user]).start()
            #time.sleep(1)
        except:
    ...
    while True:
        time.sleep(5)
        if int(activeCount()) < 2:
            f = open("follow-list.txt".format(streamer_name), "w")
            f.write(str)
            f.close()
            fd=round(followed/len(login_id_list)*100,2)
            nfd=round(notfollowed/len(login_id_list)*100,2)
            print("All Users in chat :",len(login_id_list))
            print("Followed Users :",followed,"|" , fd,"%")
            print("Not Followed Users :",notfollowed,"|" , nfd,"%")
            break
def check_follows2(user):
    try:
        global str
        global followed
        global notfollowed
...
streamer_name = input("Enter Streamer id : ")
